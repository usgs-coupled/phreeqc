			...
			for (;;) {
				...
				if (i == DIGIT) {
					...
					if (token[0] == 'D' || token[0] == 'd') {
						...
						error_msg("Expected diffuse layer thickness (m) or Debye_units (1/k) for calculating the thickness.\nCorrect is (for example): -donnan 1e-8   or   -donnan d 1.5", CONTINUE);
						error_msg(line_save, CONTINUE);
						input_error++;
						break;
					}
