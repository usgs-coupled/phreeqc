...
# else
			for (;;)
			{
				j = check_line ("Reading user_graph", FALSE, TRUE, TRUE,
								TRUE);
				if (j == EOF || j == KEYWORD)
				{
					break;
		...
		case 49:
			...
			j = check_line ("Reading after DATABASE", FALSE, TRUE, TRUE,
							TRUE);
			break;
		case 50:
		case 51:
 ...
 */
	...
	{
		sprintf (error_string,
				 "Concentration data error for %s in solution input.",
				 token1);
 ...
 */
	...
	for (;;)
	{
		...
		case OPTION_ERROR:
			input_error++;
			error_msg ("Unknown input in EXCHANGE_SPECIES keyword.",
					   CONTINUE);
 ...
 */
	...
	for (;;)
	{
		...
		case OPTION_ERROR:
			input_error++;
			error_msg ("Unknown input in INVERSE_MODELING keyword.",
					   CONTINUE);
			error_msg (line_save, CONTINUE);
			break;
		case 0:				/* solutions */
 ...
 */
	...
	else if (strcmp_nocase_arg1 (token, "ph") != 0)
	{
		...
		if (inverse_ptr->elts == NULL)
			malloc_error ();
		replace ("(+", "(", token);
		inverse_ptr->elts[inverse_ptr->count_elts].name =
			string_hsave (token);
/*
 ...
 */
	...
	if (inverse_ptr->phases == NULL)
		malloc_error ();
	inverse_ptr->phases[inverse_ptr->count_phases].name =
		string_hsave (token);
/*
 ...
 */
			...
			for (i = 1; i < count_trxn; i++)
			{
					...
					(strstr (trxn.token[i].name, "(G)") == NULL))
				{
					...
					replace ("(H2O(L)", "H2O", token1);
					trxn.token[i].s =
						s_store (token1, trxn.token[i].z, FALSE);
				}
				else
				{
					trxn.token[i].s = NULL;
 ...
 */
	...
	{
		return_value =
			check_line ("Pure phase data", FALSE, TRUE, TRUE, TRUE);
		/* empty, eof, keyword, print */
		if (return_value == EOF || return_value == KEYWORD)
		{
			break;
	...
	for (;;)
	{
		...
		case 7:				/* gases */
			while ((i = copy_token (token, &next_char, &l)) != EMPTY)
			{
				...
				if (punch.gases == NULL)
					malloc_error ();
				punch.gases[punch.count_gases - 1].name =
					string_hsave (token);
			}
			break;
		case 12:				/* inverse */
 ...
 */
	...
	for (;;)
	{
		...
		if (i == DIGIT)
		{
			...
			{
				sprintf (error_string,
						 "USE does not accept a range of numbers, %s.",
						 token);
				warning_msg (error_string);
				sprintf (error_string,
