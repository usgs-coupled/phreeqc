 ...
 */
	...
	for (i = 0; i < punch.count_gases; i++)
	{
		...
		if (punch.high_precision == FALSE)
		{
			fpunchf (sformatf ("g_%s", punch.gases[i].name), "%12.4e\t",
					 moles);
		}
		else
		{
	...
	if (punch.soln == TRUE)
	{
		...
		else
		{
			if (use.mix_in == TRUE)
			{
				if (state != TRANSPORT)
				{
					fpunchf (PHAST_NULL ("soln"), dformat,
							 use.n_mix_user_orig);
				}
				else
				{
