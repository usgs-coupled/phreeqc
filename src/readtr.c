		...
		switch (opt) {
			...
			case 40:						/* multicomponent diffusion */
				...
				if (strstr(token, "f") == token)
					...
					input_error++;
					error_msg("Expected multicomponent diffusion flag: 'true' or 'false'.", CONTINUE);
				}
				default_Dw = 1e-9;
				multi_Dpor = 0.3;
				multi_Dpor_lim = 0.0;
				...
				if (copy_token(token, &next_char, &l) == EMPTY)
					...
					if (sscanf(token, SCANFORMAT, &default_Dw) != 1) {
						input_error++;
						sprintf(error_string,"Expected default species diffusion coefficient in water at 25oC, m2/s.");
						error_msg(error_string, CONTINUE);
						break;
					}
				}
				if (copy_token(token, &next_char, &l) == EMPTY)
					...
					if (sscanf(token, SCANFORMAT, &multi_Dpor) != 1) {
						input_error++;
						sprintf(error_string,"Expected porosity to calculate diffusion coefficient.");
						error_msg(error_string, CONTINUE);
						break;
					}
				}
				if (copy_token(token, &next_char, &l) == EMPTY)
					...
					if (sscanf(token, SCANFORMAT, &multi_Dpor_lim) != 1) {
						input_error++;
						sprintf(error_string,"Expected porosity limit for diffusive transport.");
						error_msg(error_string, CONTINUE);
						break;
					}
				}
				if (copy_token(token, &next_char, &l) == EMPTY)
					...
					if (sscanf(token, SCANFORMAT, &multi_Dn) != 1) {
						input_error++;
						sprintf(error_string,"Expected exponent for porosity reduction of diffusion coefficient (Dp = Dw * (por)^n).");
						error_msg(error_string, CONTINUE);
						break;
					}
				}
